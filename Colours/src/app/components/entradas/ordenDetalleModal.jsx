...
export default function OrdenDetalleModal({ orden, onClose }) {
  ...
  const firstPayment =
    orden.Pagos && orden.Pagos.length > 0 ? orden.Pagos[0] : null;
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      yPosition += 6;
      pdf.text(
        `Fecha de creación: ${formatFecha(orden.fecha_creacion)}`,
        20,
        yPosition
      );
      ...
      pdf.setFontSize(12);
      pdf.text(
        `TOTAL DE LA ORDEN: ${formatMonto(getRealOrderTotal(orden))}`,
        20,
        yPosition
      );
      yPosition += 15;
      ...
      if (firstPayment) {
        ...
        yPosition += 6;
        pdf.text(
          `Monto (sin impuestos): ${formatMonto(firstPayment.monto)}`,
          20,
          yPosition
        );
        pdf.text(
          `Total (con impuestos): ${formatMonto(firstPayment.total)}`,
          110,
          yPosition
        );
        yPosition += 6;
        pdf.text(
          `Fecha de pago: ${formatFecha(firstPayment.fecha_pago)}`,
          20,
          yPosition
        );
        ...
        if (firstPayment.imagen && firstPayment.imagen.trim() !== "") {
          ...
          try {
            ...
            pdf.setFontSize(10);
            pdf.text(
              "⚠️ No se pudo cargar el comprobante de pago en el PDF",
              20,
              yPosition
            );
            pdf.text(
              `URL: ${firstPayment.imagen.substring(0, 60)}...`,
              20,
              yPosition + 5
            );
            yPosition += 15;
      ...
      // Guardar el PDF
      const fileName = `Orden_${orden.id.slice(0, 8)}_${new Date()
        .toISOString()
        .slice(0, 10)}.pdf`;
      pdf.save(fileName);
    } catch (error) {
                  ...
                  <div className="text-sm">
                    ...
                    {firstPayment && ( // Mostrar fecha de pago si hay un pago
                      <p className="text-gray-600">
                        ...
                        {formatFecha(firstPayment.fecha_pago)}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-3 rounded">
                    <div className="flex justify-between font-bold text-lg">
                      ...
                      <span>{formatMonto(getRealOrderTotal(orden))}</span>{" "}
                      {/* Usar el monto real */}
                    </div>
                    <div className="mt-2">
                      <span
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <div>
                          ...
                          <p className="font-medium text-green-700">
                            {firstPayment.estatus}
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-600">Referencia:</p>
                          <p className="font-medium">
                            {firstPayment.referencia}
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-600">
                          ...
                          <p className="font-medium">
                            {formatMonto(firstPayment.monto)}
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-600">
                          ...
                          <p className="font-bold text-lg">
                            {formatMonto(firstPayment.total)}
                          </p>
                        </div>
