...
export default function OrdenesYPagos() {
  ...
  const getEventoNombre = (orden) => {
    if (orden.DetalleDeOrdens && orden.DetalleDeOrdens.length > 0) {
      ...
      if (
        detalle &&
        detalle.Entrada &&
        detalle.Entrada.Evento &&
        detalle.Entrada.Evento.nombre
      ) {
  ...
  const getEstadoBadge = (estado) => {
    ...
    if (estado === "cancelado") {
      return (
        <span className="px-2 py-1 rounded-full text-xs border bg-red-800 text-red-200 border-red-600">
          Cancelado
        </span>
      );
    }
    ...
    return (
      <span className="px-2 py-1 rounded-full text-xs border bg-blue-800 text-blue-200 border-blue-600">
        {estado}
      </span>
    );
  };
  ...
  const handleSort = (field) => {
    ...
    if (!allowedSortFields.includes(field)) {
      console.warn(
        "Sorting by field " + field + " is not allowed by the backend."
      );
      return;
    }
  ...
  const fetchPaginatedOrdenes = async (page) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(
          "Error al cargar las ordenes: " + (data.message || "Desconocido")
        );
      }
    } catch (err) {
  ...
  return (
    <div className="p-4">
      <Header title="Ordenes y Pagos" />
      {error && (
        <div className="p-2 bg-red-900/50 text-red-300 text-xs rounded border border-red-700 mb-4">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        <div className="bg-transparent border border-[#BF8D6B] rounded-lg p-3 text-center">
          ...
          <p className="text-lg font-bold text-white">
            {formatMonto(summaryData.totalOrdersValue)}
          </p>
        ...
        <div className="bg-transparent border border-[#BF8D6B] rounded-lg p-3 text-center">
          ...
          <p className="text-lg font-bold text-green-400">
            {formatMonto(summaryData.totalPaidValue)}
          </p>
          <p className="text-xs text-gray-400">
            {summaryData.paidOrdersCount} pagos
          </p>
        </div>
        <div className="bg-transparent border border-[#BF8D6B] rounded-lg p-3 text-center">
          <p className="text-[#BF8D6B] text-xs mb-1">Por Cobrar</p>
          <p className="text-lg font-bold text-orange-400">
            {formatMonto(summaryData.totalPendingValue)}
          </p>
        ...
        <div className="flex flex-wrap gap-2 justify-between items-center mb-3">
          ...
          <div className="relative w-full sm:w-1/4">
            <input
              ...
              value={filters.salon}
              onChange={(e) =>
                setFilters({ ...filters, salon: e.target.value })
              }
              className="w-full p-2 bg-transparent text-white rounded border border-[#BF8D6B] placeholder-gray-400 text-xs pl-8"
            />
            <Search className="absolute left-2 top-1/2 transform -translate-y-1/2 text-[#BF8D6B] h-3 w-3" />
          ...
          <div className="border-t border-[#BF8D6B] pt-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
              <div className="relative">
                <label className="block text-xs text-[#BF8D6B] mb-1">
                  Fecha Desde
                </label>
                <input
                  type="date"
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFilters({ ...filters, metodoDePago: e.target.value })
                  }
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFilters({ ...filters, cuotas: e.target.value })
                  }
                  className="w-full p-2 bg-transparent text-white rounded border border-[#BF8D6B] placeholder-gray-400 text-xs"
                />
              </div>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full">
            <thead className="bg-[#BF8D6B]/20">
              <tr>
                <th className="px-3 py-2 text-left text-white font-medium text-xs">
                  Vendedor
                </th>
                <th
                ...
                <th className="px-3 py-2 text-center text-white font-medium text-xs">
                  Acciones
                </th>
              </tr>
            </thead>
          ...
          <div className="space-y-2">
            {ordenes.map((orden) => {
              ...
              return (
                <div
                  key={orden.id}
                  className="bg-transparent rounded border border-[#BF8D6B] overflow-hidden"
                >
                  <div className="p-3">
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 mt-4">
          ...
          {currentPage > 2 && (
            <span className="px-1 text-[#BF8D6B] text-xs">...</span>
          )}
